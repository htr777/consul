...
func (c *BootstrapConfig) ConfigureArgs(args *BootstrapTplArgs, omitDeprecatedTags bool) error {
	...
	if c.PrometheusBindAddr != "" {
		...
		// generateListenerConfig() to create a cluster called
		// "prometheus_backend" (rather than the "self_admin" cluster it usually
		// creates) which the listener will point to.
...
func (c *BootstrapConfig) generateListenerConfig(args *BootstrapTplArgs, bindAddr, name, matchType, matchValue, prefixRewrite, prometheusBackendPort string) error {
	...
	// If a prometheusBackendPort is set, create "prometheus_backend" cluster
	// with the prometheusBackendPort that the listener will point to, rather
	// than the "self_admin" cluster. This is for the merged metrics feature in
	// consul-k8s, so the envoy_prometheus_bind_addr listener will point to the
	// merged Envoy and service metrics endpoint rather than the Envoy admin
	// endpoint for metrics. This cluster will only be created once since it's
	// only created when prometheusBackendPort is set, and prometheusBackendPort
	// is only set when calling this function if c.PromBindAddr is set.
